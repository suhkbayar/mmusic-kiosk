...
const Receipt = forwardRef<HTMLDivElement, ReceiptProps>((props, ref) => {
  const { order, qrUrl, withVat, participant } = props;
  ...
  return (
    <div ref={ref} className="w-[80mm] text-[10px] p-[2mm] h-full bg-white text-black  ">
      ...
      <div className="flex justify-between my-2">
        <span className="text-lg font-bold"> Дугаар: {order?.number.slice(8)} </span>
        <span className="text-lg font-bold">{t(`mainPage.${order.type}`)}</span>
      </div>
      <div className="border-b  my-1"></div>
      <div className="mt-2">
      ...
      <div className="mt-2">
        <span>Ширээ: {order?.table?.name} </span>
      </div>
